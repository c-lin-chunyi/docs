...
Here $Y_{i,j,k}$ denotes the random score that would be generated by the model. Once data are observed, we write the observed value as $y_{i,j,k}$.
...
| $\hat{\boldsymbol{\alpha}}\perp\hat{\boldsymbol{\beta}}$ | balanced factorization|
...
| $\hat{\boldsymbol{\varepsilon}}\perp$ all four others | cell-wise $\sum_k\widehat{\varepsilon}_{(i,j,k)}=0$ |
